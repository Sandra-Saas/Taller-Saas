...
function extractIdentityFromPayload(payload) {
  ...
  return {
    ...
    tenantId: metadata.tenant_id || appMetadata.tenant_id || payload.tenant_id || null,
...
export async function getRequestContext(req, options = {}) {
  ...
  const resolvedTenantId = user?.tenantId || identity.tenantId || null
  const tenant = user?.tenant || (await resolveTenant(resolvedTenantId))
  const fallbackUser = user || (await resolveFallbackUser(tenant?.id || resolvedTenantId))

  return {
    authType: 'session',
    tenantId: tenant?.id || user?.tenantId || fallbackUser?.tenantId || identity.tenantId || null,
    tenant: tenant || null,
    userId: user?.id || fallbackUser?.id || null,
